...
public class MainFrame extends JFrame {
	...
	public MainFrame(String addr, String pass, String smtp_addr, String pop_addr,LoginDialog loginDialog) {
		...
		setTitle("Simple Mail client");
		// 统一字体
		Font font = new Font("Microsoft elegant black", Font.PLAIN, 17);
		...
		tabbedPane.addTab("write a letter", new ImageIcon("write.png"), panel_write);
		panel_write.setLayout(null);

		JLabel label = new JLabel("Recipient address：");
		label.setFont(new Font("Microsoft elegant black", Font.PLAIN, 18));
		label.setBounds(26, 13, 129, 18);
		panel_write.add(label);

		addrs_text = new JTextField();
		addrs_text.setToolTipText("Please use the English semicolon for multiple addresses");
		addrs_text.setFont(new Font("Microsoft elegant black", Font.PLAIN, 14));
		addrs_text.setBounds(137, 12, 412, 24);
		panel_write.add(addrs_text);
		addrs_text.setColumns(10);

		JLabel label_1 = new JLabel("Theme ：");
		label_1.setFont(new Font("Microsoft elegant black", Font.PLAIN, 18));
		label_1.setBounds(26, 44, 77, 18);
		panel_write.add(label_1);

		subject_text = new JTextField();
		subject_text.setToolTipText("");
		subject_text.setFont(new Font("Microsoft elegant black", Font.PLAIN, 14));
		...
		tabbedPane.addTab("Received", new ImageIcon("box.png"), panel_box);
			...
			public Object getValueAt(int rowIndex, int columnIndex) {
				if (columnIndex == 0) {
					return mails.get(rowIndex).getTo_list().size() > 0 ? mails
							.get(rowIndex).getTo_list().get(0) : "anonymous";
				}
				if (columnIndex == 1)
	...
	protected void initSmtpAndSend() {
		...
		String message = smtpController.send(mail) ? "Sent successfully" : "Failed to send\n";
		smtpController.quitSmtpServer();
		JOptionPane.showMessageDialog(MainFrame.this, message);
	}

	protected void doSave() {
		String fileName = "mail_"+mails.get(curMailIndex).getSubject() + System.currentTimeMillis() +".txt";
		...
		showMyDialog("To save to a file:"+file.getAbsolutePath());
	...
	protected void doReply() {
		if(mails.get(curMailIndex).getTo_list().size() == 0) JOptionPane.showMessageDialog(MainFrame.this, "The user is anonymous，Can not reply！");
		else{
			setTableVisible();
			tabbedPane.setSelectedIndex(0);
			addrs_text.setText(MyUtility.getStringFromTo_list(mails.get(curMailIndex).getTo_list()));
			subject_text.setText("Reply Mail:"+mails.get(curMailIndex).getSubject());
		}
	}

	protected void doDelete() {
		if(popController.deleteItem(curMailIndex+"")){
			showMyDialog("successfully deleted！");
			setTableVisible();
			//刷新数据
			initPop();
		}else {
			showMyDialog("failed to delete！");
		}
	}
